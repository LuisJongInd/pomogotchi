...
static void SPI_PeripheralClockControl(SPI_TypeDef *pSPIx, EnableDisable EnorDi);
...
DriverStatus SPI_Init(SPI_DriverTypeDef *pSPIDriver)
{
  ...
  }else if ( pSPIDriver->Config.Type == SPI_Type_RxOnly )
  {
    // BIDIMODE[0] = 0
    pSPIDriver->pSPIx->CR1 &= ~(SPI_CR1_BIDIMODE);
    // RXONLY[0] is used with BIDIMODE to select the direction of transfer in
    // 2-line mode. 
  ...
  pSPIDriver->pSPIx->CR1 |= ( pSPIDriver->Config.Hierarchy  << SPI_CR1_MSTR_Pos );
  ...
  pSPIDriver->pSPIx->CR1 |= ( pSPIDriver->Config.FrameFormat << SPI_CR1_LSBFIRST_Pos);
  ...
  pSPIDriver->pSPIx->CR1 |= ( pSPIDriver->Config.DataFormat << SPI_CR1_DFF_Pos);
